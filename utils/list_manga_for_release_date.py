...
listable_query = (
    f"WITH dmd AS ( "
    f"select \"DIAMD_NO\", \"SHORT_ISBN_NO\", \"EAN_NO\" from dmd_master_data "
    f"where \"SHORT_ISBN_NO\" is not null AND \"EAN_NO\" is not null "
    f"group by \"DIAMD_NO\", \"SHORT_ISBN_NO\", \"EAN_NO\" "
    f"order by \"DIAMD_NO\" "
    f") "
    f"SELECT inv.issue_id as item_code, inv.units, inv.title, inv.retail_price, inv.description, pub.name as publisher, stock.stock_number, dmd.\"SHORT_ISBN_NO\", dmd.\"EAN_NO\" "
    f"FROM inventory AS inv "
    f"INNER JOIN publisher as pub ON inv.publisher_id = pub.id "
    f"INNER JOIN item_code_stock_xref as stock ON inv.issue_id = stock.item_code "
    f"LEFT JOIN inventory_ebay as ebay ON inv.issue_id = ebay.issue_id "
    f"LEFT JOIN dmd on inv.issue_id = dmd.\"DIAMD_NO\" "
    f"WHERE inv.units > 0 "
    f"AND inv.hidden = false "
    f"AND inv.release_date = '{release_date}' "
    f"AND pub.manga = true "
    f"AND ebay.ebay_item_id IS NULL"
)
listable = pd.read_sql(listable_query, connection)
#listable = listable.drop_duplicates()
...
for record in listable.to_dict('records'):
    ...
    We guarantee that this item will be in new and unread condition with little or no cosmetic defects.
    If you have any questions about the condition of an item that will be sent to you, simply ask and we will be happy to provide photos.<br><br>
